...
const BROWSER_SEARCH_PROVIDERS = Object.freeze({
  ...
  baidu_browser: Object.freeze({
    id: "baidu_browser",
    engine: "baidu",
    label: "Baidu Browser",
    baseUrl: "https://www.baidu.com/s",
    params: (query, maxResults) => ({ wd: query, rn: String(maxResults) }),
  }),
});
...
function buildBrowserSearchExtractionScript(provider, maxResults = 5) {
  ...
  return `(() => {
    ...
    function cleanUrl(raw) {
      ...
      if (url.hostname.endsWith("baidu.com") && url.pathname === "/link") {
    ...
    function hasNoResultsSignals() {
      ...
      if (engine === "baidu") {
        return (
          bodyText.includes("很抱歉，没有找到") ||
          bodyText.includes("没有找到与")
        );
      }
      return false;
    }
    ...
    function baiduResults() {
      const items = Array.from(document.querySelectorAll("div.result.c-container, div.result-op.c-container"));
      return items.map((item, idx) => {
        const anchor = firstAnchor(item, ["h3 a", "a"]);
        const title = firstText(item, ["h3"]);
        if (!title || !anchor) return null;
        const snippet = firstText(item, [".c-abstract", ".c-span-last", ".content-right_8Zs40", "[class*=abstract]"]);
        ...
        return {
          ...
          rank: idx + 1,
          score: null,
          metadata: { display_url: displayUrl(rawUrl), engine },
        };
      }).filter(Boolean);
    }
    ...
    if (!blocked) {
      ...
      else if (engine === "baidu") results = baiduResults();
    }
    let status = "ok";
